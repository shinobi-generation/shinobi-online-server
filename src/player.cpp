...
Player::Player(const std::string &_name, ProtocolGame *p) :
    ...
    for (int8_t i = 0; i < sizeof(talkState); i++)
        talkState[i] = false;
#ifdef __ENABLE_SERVER_DIAGNOSTIC__
...
int32_t Player::getDefense() const {
    ...
    return ((int32_t) std::ceil(
            ((float) (defenseSkill * (defenseValue * 0.015)) + (defenseValue * 0.1)) * defenseFactor));
}
...
float Player::getDefenseFactor() const {
    switch (fightMode) {
        ...
        case FIGHTMODE_DEFENSE: {
            if ((OTSYS_TIME() - lastAttack) <
                const_cast<Player *>(this)->getAttackSpeed()) //attacking will cause us to get into normal defense
                return 1.0f;
...
void Player::setSkillLevel(skills_t skill, uint32_t value) {
    ...
    for (auto &advanceEvent: advanceEvents)
        advanceEvent->executeAdvance(this, skill, old_level, skills[skill][SKILL_LEVEL]);
...
void Player::addSkillAdvance(skills_t skill, uint32_t count, bool useMultiplier/* = true*/) {
    ...
    while (skills[skill][SKILL_TRIES] + count >= nextReqTries) {
        ...
        CreatureEventList advanceEvents = getCreatureEvents(CREATURE_EVENT_ADVANCE);
        for (auto &advanceEvent: advanceEvents)
            advanceEvent->executeAdvance(this, skill, (skills[skill][SKILL_LEVEL] - 1), skills[skill][SKILL_LEVEL]);
...
bool Player::canOpenCorpse(uint32_t ownerId) {
    return getID() == ownerId || (party && party->canOpenCorpse(ownerId)) ||
           hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges);
}
...
void Player::dropLoot(Container *corpse) {
    ...
    uint32_t start = g_config.getNumber(
            ConfigManager::BLESS_REDUCTION_BASE), loss = lossPercent[LOSS_CONTAINERS], bless = getBlessings();
    ...
    for (int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i) {
        ...
        if (skull > SKULL_WHITE || (item->getContainer() && rand < loss) ||
            (!item->getContainer() && rand < itemLoss)) {
...
bool Player::setStorage(const uint32_t key, const std::string &value) {
    ...
    if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
        ...
        std::cout << "[Warning - Player::setStorage] Unknown reserved key: " << key << " for player: " << getName()
                  << std::endl;
...
void Player::eraseStorage(const uint32_t key) {
    ...
    if (IS_IN_KEYRANGE(key, RESERVED_RANGE))
        std::cout << "[Warning - Player::eraseStorage] Unknown reserved key: " << key << " for player: " << name
                  << std::endl;
}
...
void
Player::sendUpdateContainerItem(const Container *container, uint8_t slot, const Item *oldItem, const Item *newItem) {
    if (!client)
        return;
...
void Player::onCreatureMove(const Creature *creature, const Tile *newTile, const Position &newPos,
    ...
    if (tradeState != TRADE_TRANSFER &&
        ((tradeItem && !Position::areInRange<1, 1, 0>(tradeItem->getPosition(), getPosition()))
...
void Player::onUpdateContainerItem(const Container *container, uint8_t slot,
                                   const Item *oldItem, const ItemType &oldType, const Item *newItem,
...
bool Player::isMuted(uint16_t channelId, SpeakClasses type, uint32_t &time) {
    ...
    return time > 0 && type != SPEAK_PRIVATE_PN &&
           (type != SPEAK_CHANNEL_Y || (channelId != CHANNEL_GUILD && !g_chat.isPrivateChannel(channelId)));
}
...
void Player::removeMessageBuffer() {
    ...
    if (!hasFlag(PlayerFlag_CannotBeMuted) && maxBuffer != 0 && messageBuffer <= maxBuffer + 1) {
        if (++messageBuffer > maxBuffer) {
            ...
            if (Condition *condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_MUTED,
                                                                  muteTime * 1000))
                addCondition(condition);
...
void Player::drainHealth(Creature *attacker, CombatType_t combatType, int32_t damage) {
    ...
    char buffer[150];
    if (attacker)
        sprintf(buffer, "You lose %d hitpoint%s due to an attack by %s.", damage, (damage != 1 ? "s" : ""),
                attacker->getNameDescription().c_str());
    else
...
BlockType_t Player::blockHit(Creature *attacker, CombatType_t combatType, int32_t &damage,
    ...
    if (damage > 0) {
        ...
        for (int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot) {
            ...
            if (it.abilities.reflect[REFLECT_PERCENT][combatType] &&
                it.abilities.reflect[REFLECT_CHANCE][combatType] < random_range(0, 100)) {
                reflected += (int32_t) std::ceil(
                        (double) (damage * it.abilities.reflect[REFLECT_PERCENT][combatType]) / 100.);
...
bool Player::onDeath() {
    ...
    if (preventLoss) {
        ...
        if (preventLoss->getCharges() > 1) //weird, but transform failed to remove for some hosters
            g_game.transformItem(preventLoss, preventLoss->getID(),
                                 std::max(0, ((int32_t) preventLoss->getCharges() - 1)));
        else
    ...
    if (preventDrop && preventDrop != preventLoss) {
        if (preventDrop->getCharges() > 1) //weird, but transform failed to remove for some hosters
            g_game.transformItem(preventDrop, preventDrop->getID(),
                                 std::max(0, ((int32_t) preventDrop->getCharges() - 1)));
        else
            g_game.internalRemoveItem(NULL, preventDrop);
    }
    ...
    if (skillLoss) {
        ...
        for (int16_t i = 0; i < 7; ++i) //for each skill
        {
            ...
            lostSkillTries = (uint32_t) std::ceil(
                    sumSkillTries * ((double) (percent * lossPercent[LOSS_SKILLS]) / 100.));
...
void Player::addExhaust(uint32_t ticks, Exhaust_t type) {
    if (Condition *condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_EXHAUST, ticks, 0, false,
                                                          type))
        addCondition(condition);
}
...
void Player::addInFightTicks(bool pzLock/* = false*/) {
    ...
    if (Condition *condition = Condition::createCondition(CONDITIONID_DEFAULT,
                                                          CONDITION_INFIGHT,
                                                          g_config.getNumber(ConfigManager::PZ_LOCKED)))
        addCondition(condition);
}
...
ReturnValue Player::__queryAdd(int32_t index, const Thing *thing, uint32_t count, uint32_t flags) const {
    ...
    bool childIsOwner = ((flags & FLAG_CHILDISOWNER) == FLAG_CHILDISOWNER), skipLimit = ((flags & FLAG_NOLIMIT) ==
                                                                                         FLAG_NOLIMIT);
    if (childIsOwner) {
...
Cylinder *Player::__queryDestination(int32_t &index, const Thing *thing, Item **destItem,
    ...
    if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER) {
        *destItem = NULL;
        ...
        for (std::list<std::pair<Container *, int32_t> >::iterator dit = deepList.begin();
             dit != deepList.end(); ++dit) {
...
double Player::getGainedExperience(Creature *attacker) const {
    ...
    if ((min > 0 && level < (uint32_t) std::floor(attackerLevel * min)) || (max > 0 &&
                                                                            level >
                                                                            (uint32_t) std::floor(attackerLevel * max)))
        return 0;
...
void Player::onAddCondition(ConditionType_t type, bool hadCondition) {
    ...
    if (getLastPosition().x && type !=
                               CONDITION_GAMEMASTER) // don't send if player have just logged in (its already done in protocolgame), or condition have no icons
        sendIcons();
}
...
void Player::onAttackedCreatureDrain(Creature *target, int32_t points) {
    ...
    if (party && target && (!target->getMaster() || !target->getMaster()->getPlayer())
        && target->getMonster() &&
        target->getMonster()->isHostile()) //we have fulfilled a requirement for shared experience
...
void Player::onSummonAttackedCreatureDrain(Creature *summon, Creature *target, int32_t points) {
    ...
    char buffer[100];
    sprintf(buffer, "Your %s deals %d damage to %s.", summon->getName().c_str(), points,
            target->getNameDescription().c_str());
    sendTextMessage(MSG_EVENT_DEFAULT, buffer);
}
...
bool Player::onKilledCreature(Creature *target, uint32_t &flags) {
    ...
    if (hasFlag(PlayerFlag_NotGainInFight) || !hasBitSet((uint32_t) KILLFLAG_JUSTIFY, flags) ||
        getZone() != target->getZone())
        return true;

    Player *targetPlayer = target->getPlayer();
    if (!targetPlayer || Combat::isInPvpZone(this, targetPlayer) || !hasCondition(CONDITION_INFIGHT) ||
...
void Player::generateReservedStorage() {
    ...
    for (OutfitMap::const_iterator it = outfits.begin(); it != outfits.end(); ++it) {
        ...
        std::cout << "[Warning - Player::genReservedStorageRange] Player " << getName()
                  << " with more than 500 outfits!" << std::endl;
        break;
    }
}
...
uint64_t Player::getLostExperience() const {
    ...
    double percent =
            (double) (lossPercent[LOSS_EXPERIENCE] - vocation->getLessLoss() - (getBlessings() * g_config.getNumber(
...
void Player::unlearnInstantSpell(const std::string &name) {
    ...
    LearnedInstantSpellList::iterator it = std::find(learnedInstantSpellList.begin(), learnedInstantSpellList.end(),
                                                     name);
...
void Player::manageAccount(const std::string &text) {
    ...
    switch (accountManager) {
            ...
            if (!talkState[1]) {
                ...
                if (!IOLoginData::getInstance()->playerExists(managerString, true)) {
                    ...
                    if (IOLoginData::getInstance()->getGuidByName(tmp, managerString2) &&
                        ...
                        talkState[2] = false;
                        msg
                                << "Your character has been successfully renamed, you should now be able to login at it without any problems.";
                    } else {
                        talkState[1] = talkState[2] = false;
        ...
        case MANAGER_ACCOUNT: {
            ...
            if (checkText(text, "cancel") || (checkText(text, "account") && !talkState[1])) {
                ...
                msg
                        << "Do you want to change your 'password', request a 'recovery key', add a 'character', or 'delete' a character?";
                ...
                if (!isValidName(tmp, false))
                    msg
                            << "That name contains invalid characters, try to say your name again, you might have typed it wrong.";
                else {
                    talkState[2] = false;
                    talkState[3] = true;
                    managerString = tmp;
                ...
                switch (IOLoginData::getInstance()->deleteCharacter(managerNumber, managerString)) {
                    case DELETE_INTERNAL:
                        msg
                                << "An error occured while deleting your character. Either the character does not belong to you or it doesn't exist.";
                        break;
                    ...
                    case DELETE_HOUSE:
                        msg
                                << "Your character owns a house. To make sure you really want to lose your house by deleting your character, you have to login and leave the house or pass it to someone else first.";
                        break;

                    case DELETE_LEADER:
                        msg
                                << "Your character is the leader of a guild. You need to disband or pass the leadership someone else to delete your character.";
                        break;

                    case DELETE_ONLINE:
                        msg
                                << "A character with that name is currently online, to delete a character it has to be offline.";
                        break;
                }
                ...
                if (tmp.length() < 6)
                    msg
                            << "That password is too short, at least 6 digits are required. Please select a longer password.";
                ...
                if (account.charList.size() <= 15) {
                    ...
                    msg
                            << "Your account reach the limit of 15 players, you can 'delete' a character if you want to create a new one.";
                }
            } else if (talkState[6]) {
                managerString = text;
                trimString(managerString);
                ...
                if (g_config.getBool(ConfigManager::START_CHOOSEVOC)) {
                    ...
                    bool firstPart = true;
                    for (VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation();
                         it != Vocations::getInstance()->getLastVocation(); ++it) {
                    ...
                    if (IOLoginData::getInstance()->createCharacter(managerNumber, managerString, managerNumber2,
                                                                    (uint16_t) managerSex))
                ...
                for (VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation();
                    ...
                    if (checkText(text, tmp) && it != Vocations::getInstance()->getLastVocation() &&
                ...
                msg
                        << "Please read the latest message that I have specified, I don't understand the current requested action.";
        ...
        case MANAGER_NEW: {
            if (checkText(text, "account") && !talkState[1]) {
                ...
                if (g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER)) {
                    do
                        sprintf(managerChar, "%d%d%d%d%d%d%d", random_range(2, 9), random_range(2, 9),
                                random_range(2, 9), random_range(2, 9), random_range(2, 9), random_range(2, 9),
                                random_range(2, 9));
                ...
                if (tmp.length() < 3)
                    msg
                            << "That account name is too short, at least 3 digits are required. Please select a longer account name.";
                else if (tmp.length() > 25)
                    msg
                            << "That account name is too long, not more than 25 digits are required. Please select a shorter account name.";
                ...
                managerString2 = text;
                if (IOLoginData::getInstance()->validRecoveryKey(managerNumber, managerString2) &&
                    managerString2 != "0") {
                    sprintf(managerChar, "%s%d", g_config.getString(ConfigManager::SERVER_NAME).c_str(),
                            random_range(100, 999));
    ...
    if (!noSwap)
        sendTextMessage(MSG_STATUS_CONSOLE_ORANGE,
                        "Hint: Type 'account' to manage your account and if you want to start over then type 'cancel'.");
}
