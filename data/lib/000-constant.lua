...
COMBAT_JUUKEN = 3000
COMBAT_KIKAICHUU = 3001
COMBAT_RESISTENCE = 3002
COMBAT_KAGEMANE = 3003
...
CONST_ANI_NONE = 255
CONST_ANI_LAST = CONST_ANI_ENDAN
...
return {x=pos.x+delta.x,y=pos.y+delta.y,z=pos.z}
end

function distBetween(cid1,cid2)
return math.max(math.abs(getThingPos(cid1).x-getThingPos(cid2).x), math.abs(getThingPos(cid1).y-getThingPos(cid2).y))
end
...
function getAreaWidthAndHeight(area) return #(area[1]),#area end
...
for n=1,table.getn(area) do
if type(area[n]) ~= "table" then
return 0
end
if table.getn(area[n]) == 0 then
return 0
end
for n2=1,table.getn(area[n]) do
...
if itemid == 670 or itemid == 671 or (itemid >= 6580 and itemid <= 6608) or (itemid >= 6613 and itemid <= 6626) or itemid == 6838 or (itemid >= 6683 and itemid <= 6686) or itemid == 7200 or itemid == 7236 then
return 1
else
return 0
end
...
if (itemid >= 4632 and itemid <= 4663) or (itemid >= 4828 and itemid <= 4831) or (itemid >= 494 and itemid <= 505) or (itemid >= 6627 and itemid <= 6674) or (itemid >= 6687 and itemid <= 6694) then
return 1
else
return 0
end
...
if (itemid >= 4608 and itemid <= 4625) or (itemid >= 4664 and itemid <= 4666) or (itemid >= 4820 and itemid <= 4825) then
return 1
else
return 0
end
...
function evento(p)
if isPlayer(p.id) == 1 then
doCombat(p.id, p.combat, p.var)
end
end

di1 = {[5]=1,[4]=2,[6]=3,[7]=4}
di2 = {5,4,6,7,5,4,6,7,5,4,6,7}
