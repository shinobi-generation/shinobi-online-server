...
local info = {
	...
	if getCreatureStorage(cid, sto_hachimon[1]) == 0 then
		doPlayerSendCancel(cid, "You can't use Kyumon while you are using Kaimon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[3]) == 0 then
		doPlayerSendCancel(cid, "You can't use Kyumon while you are using Seimon.")
		return true
	end
	...
	if getCreatureStorage(cid, sto_hachimon[5]) == 0 then
		doPlayerSendCancel(cid, "You can't use Kyumon while you are using Tomon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[6]) == 0 then
		doPlayerSendCancel(cid, "You can't use Kyumon while you are using Keimon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[7]) == 0 then
		doPlayerSendCancel(cid, "You can't use Kyumon while you are using Kyomon.")
		return true
	end
	...
	if getPlayerStorageValue(cid, sto_hachimon[2]) > 0  then
		...
		setPlayerStorageValue(cid, sto_hachimon[2], 0)
		removeStaminaSecondLife(cid, -manaPercent, 1, sto_hachimon[2])
	elseif getPlayerStorageValue(cid, sto_hachimon[2]) == 0 then
