...
local info = {
	...
	if getCreatureStorage(cid, sto_hachimon[1]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomon while you are using Kaimon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[2]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomon while you are using Kyumon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[3]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomonn while you are using Seimon.")
		return true
	end
	...
	if getCreatureStorage(cid, sto_hachimon[6]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomon while you are using Keimon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[7]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomon while you are using Kyomon.")
		return true
	end
	
	if getCreatureStorage(cid, sto_hachimon[8]) == 0 then
		doPlayerSendCancel(cid, "You can't use Tomon while you are using Shimon.")
		return true
	end
	if getPlayerStorageValue(cid, sto_hachimon[5]) > 0 then
		...
		addEvent(doCreatureSay, 1200, cid, "KAI!!!", TALKTYPE_SAY)
		hachimonTonkou2(cid)
		doPlayerSetSkillLevel(cid, 0, getPlayerSkill(cid, 0)+10)
		doPlayerSetSkillLevel(cid, 6, getPlayerSkill(cid, 6)+10)
		setPlayerStorageValue(cid, sto_hachimon[5], 0)
		doChangeSpeed(cid, -getCreatureSpeed(cid))
		doChangeSpeed(cid, getCreatureBaseSpeed(cid) + info.speed)
		removeStaminaSecondLife(cid, -manaPercent, 1, sto_hachimon[5])
	elseif getPlayerStorageValue(cid, sto_hachimon[5]) == 0 then
