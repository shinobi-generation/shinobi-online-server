...
local function registerPlayerInQuest(params)      
      setPlayerStorageValue(params.player, "hasActiveInQuest1", 1)
      setPlayerStorageValue(params.player, "genericQuestString1", params.globalStorage)
      registerPosInStorage(params.player, "genericQuestPos1", params.posExit)
end
...
local const = {
     exitPos = {x=2540,y=1687,z=8},
     enterPos = {x=2540,y=1683,z=8},
     storageName = 8794,
...
local function stoping()
      ...
      if isCreature(player) then
            setPlayerStorageValue(player, "hasActiveInQuest1", -1)
...
local function sendEffect(pos, eff, isFinal, initialTime)
      if const.isOpening == false or initialTime ~= const.totalTime then
            return
      end
...
function onStepIn(cid, item, position, fromPosition)
      if item.actionid == 19191 then
            if not const.isOpening then
                  starting(cid)
            else         
            end
      else
            if const.isOpening == false or const.currentPlayer ~= cid then
                  return true
            end
